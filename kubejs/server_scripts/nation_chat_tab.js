        ...
        Commands.literal('n')
            .then(Commands.argument('message', StringArgumentType.greedyString())
                .executes(function(ctx) {
                    ...
                    if (!player) return 0
                    var text = StringArgumentType.getString(ctx, 'message')
                    return sendNationChatMessage(player, text) > 0 ? 1 : 0
                })
            )
            .executes(function(ctx) {
                var player = ctx.source.player
                if (player) {
                    sendMsg(player, 'Chat Nation', 'Usage : §e/n <message> §7(ou §e/nc §7pour basculer automatiquement)', '§7')
                }
                return 0
            })
    )

    // Raccourci direct /nc [global|nation] (toggle mode)
    event.register(
        Commands.literal('nc')
            .executes(function(ctx) {
                var player = ctx.source.player
                if (player) setPlayerChatMode(player, null)
                return 1
            })
            .then(Commands.argument('canal', StringArgumentType.string())
                .executes(function(ctx) {
                    var player = ctx.source.player
                    if (!player) return 0
                    var channel = StringArgumentType.getString(ctx, 'canal').toLowerCase()
                    setPlayerChatMode(player, channel)
                    return 1
                })
            )
    )

    // Sous-commandes /nation chat et /nation msg
    event.register(
...
// Vérification périodique légère toutes les 2 secondes (40 ticks)
