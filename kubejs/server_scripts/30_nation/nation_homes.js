...
function setNationHome(arg1, arg2) {
    var player = arg2 || arg1
    var team = arg2 ? arg1 : getPlayerNationTeam(player)
    if (!player) return 0
    try {
        if (!team) {
...
function deleteNationHome(arg1, arg2) {
    var player = arg2 || arg1
    var team = arg2 ? arg1 : getPlayerNationTeam(player)
    if (!player) return 0
    try {
...
function teleportPlayerToCoords(player, homeData, label) {
    ...
    try {
        // 0. Interdiction absolue si le joueur porte un Étendard National
        if (typeof hasPlayerNationFlag === 'function' && hasPlayerNationFlag(player)) {
            sendMsg(player, 'Téléportation', 'Interdit : Vous portez un Étendard National ! Vous devez vous déplacer à pied ou en véhicule.', '§c')
