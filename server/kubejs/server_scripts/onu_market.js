...
var ONU_MARKET_CONFIG = {
    hubX: -204,
    hubZ: -172,
    maxDistance: 200,            // Rayon maximal d'accès physique (200 blocs)
    rebalanceIntervalTicks: 36000, // Rééquilibrage toutes les 30 minutes (36 000 ticks)
    spreadPercent: 30,           // Spread d'achat/revente : rachat à 70% du cours de vente
    rebalanceRate: 0.15          // Convergence de 15% vers le stock cible à chaque cycle
}
...
// -----------------------------------------------------------------------------
// CATALOGUE OFFICIEL DES MATIÈRES PREMIÈRES ET RESSOURCES RARES
// -----------------------------------------------------------------------------
var ONU_MARKET_CATALOG = [
...
// -----------------------------------------------------------------------------
// COMMANDES CLI : /marche, /bourse & ADMINISTRATION
// -----------------------------------------------------------------------------
ServerEvents.commandRegistry(function(event) {
    ...
    var registerMarketCmd = function(cmdName) {
        event.register(
            Commands.literal(cmdName)
                .executes(function(ctx) {
                    if (ctx.source.player) {
                        openMarketGUI(ctx.source.player)
                    }
                    return 1
                })
                .then(Commands.literal('admin')
                    .requires(function(source) { return source.hasPermission(2) })
                    .then(Commands.literal('rebalance')
                        .executes(function(ctx) {
                            var ch = rebalanceMarketStocks(ctx.source.server, false)
                            if (ctx.source.player) {
                                ctx.source.player.tell(Text.of('§8[§bONU§8] §aRééquilibrage forcé des marchés exécuté (' + ch + ' articles ajustés).'))
                            }
                            return 1
                        })
                    )
                    .then(Commands.literal('reset')
                        .executes(function(ctx) {
                            loadMarketState()
                            for (var r = 0; r < ONU_MARKET_CATALOG.length; r++) {
                                ONU_MARKET_CATALOG[r].currentStock = ONU_MARKET_CATALOG[r].targetStock
                            }
                            saveMarketState()
                            if (ctx.source.player) {
                                ctx.source.player.tell(Text.of('§8[§bONU§8] §aTous les stocks de la Bourse ont été réinitialisés à leur valeur cible.'))
                            }
                            return 1
                        })
                    )
                    .then(Commands.literal('setstock')
                        .then(Commands.argument('item', StringArgumentType.string())
                            .then(Commands.argument('quantite', IntegerArgumentType.integer(0, 100000))
                                .executes(function(ctx) {
                                    var itName = StringArgumentType.getString(ctx, 'item')
                                    var qty = IntegerArgumentType.getInteger(ctx, 'quantite')

                                    var matched = null
                                    for (var s = 0; s < ONU_MARKET_CATALOG.length; s++) {
                                        var cItem = ONU_MARKET_CATALOG[s]
                                        if (cItem.id === itName || cItem.id === 'minecraft:' + itName) {
                                            matched = cItem
                                            break
                                        }
                                    }
                                    ...
                                    if (!matched) {
                                        if (ctx.source.player) {
                                            ctx.source.player.tell(Text.of('§8[§bONU§8] §cArticle introuvable dans le catalogue : ' + itName))
                                        }
                                        return 0
                                    }
                                    ...
                                    saveMarketState()
                                    if (ctx.source.player) {
                                        ctx.source.player.tell(Text.of('§8[§bONU§8] §aStock de §e' + matched.name + ' §amis à jour : §e' + qty))
                                    }
                                    return 1
                                })
                            )
    ...
    registerMarketCmd('marche')
    registerMarketCmd('bourse')
})

// -----------------------------------------------------------------------------
// CYCLE PÉRIODIQUE DE RÉÉQUILIBRAGE (30 MINUTES)
// -----------------------------------------------------------------------------
var onuMarketTickCounter = 0

ServerEvents.tick(function(event) {
    onuMarketTickCounter++
    if (onuMarketTickCounter >= ONU_MARKET_CONFIG.rebalanceIntervalTicks) {
        onuMarketTickCounter = 0
        rebalanceMarketStocks(event.server, false)
    }
})
